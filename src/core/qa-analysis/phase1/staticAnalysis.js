...
export async function extractStaticNodes(page, opts = {}) {
  ...
  return page.evaluate((cfg) => {
    ...
    let counter   = 0;
    let loopIter  = 0;   // raw iteration counter (incremented for every element, kept or dropped)

    // Detect top-layer overlays once before the loop.
    const topLayerOverlays = detectTopLayerOverlays();

    for (const el of document.querySelectorAll('*')) {
      // ── Nav-defense early exit ──────────────────────────────────────────────
      // Every 80 elements check whether the navigation defense has detected a
      // blocked attempt SINCE we started this loop.  If yes, return the partial
      // result we have so far rather than risking losing everything when the
      // execution context is eventually destroyed.
      loopIter++;
      if (loopIter % 80 === 0 && window.__navDefense && window.__navDefense.blocked > 0) {
        const recentBlock = window.__navDefense.attempts.some(
          (a) => a.ts > Date.now() - 3000
        );
        if (recentBlock) {
          return {
            nodes, droppedNodes, visibilityMismatches,
            partial: true,
            partialReason: 'nav_defense_blocked_during_extraction',
          };
        }
      }
      const tag = el.tagName.toLowerCase();
...
/**
 * Lightweight fallback DOM extraction used when full extractStaticNodes fails
 * due to execution-context destruction or SPA navigation during analysis.
 *
 * Instead of walking every element with `querySelectorAll('*')`, this queries
 * only specific semantic tag names and role selectors.  Each element is
 * processed independently — one failure does not abort the rest.  The evaluate
 * is faster and shorter, reducing the window in which a navigation can
 * interrupt it.
 *
 * Returned nodes carry `extractedVia: 'lite'` so consumers can distinguish
 * lite-mode output from full extraction output.
 *
 * @param {import('playwright').Page} page
 * @returns {Promise<{ nodes: object[], droppedNodes: object[], visibilityMismatches: object[], extractedVia: string }>}
 */
export async function extractStaticNodesLite(page) {
  try {
    // page.evaluate() may resolve undefined (not throw) when context is destroyed
    const result = await page.evaluate(() => {
      // Semantic selectors only — fast, no full-DOM walk required.
      // Combined into one querySelectorAll call to avoid per-tag round-trips.
      const SELECTOR = [
        'a[href]',
        'button',
      ...
      try {
        ...
        return { nodes, droppedNodes: [], visibilityMismatches: [], extractedVia: 'lite_failed' };
      }
      ...
      for (const el of elements) {
        try {
          ...
          try {
            if (el.id) selectorHint = `#${CSS.escape(el.id)}`;
            else {
              const cls = Array.from(el.classList).slice(0, 2);
              if (cls.length) selectorHint = `${tag}.${cls.map((c) => CSS.escape(c)).join('.')}`;
            }
          } catch (_) {}
          ...
          // Individual element error — skip and continue
        }
      }

      return { nodes, droppedNodes: [], visibilityMismatches: [], extractedVia: 'lite' };
    });
    // evaluate() can resolve undefined (not throw) when context is destroyed mid-call
    return result ?? { nodes: [], droppedNodes: [], visibilityMismatches: [], extractedVia: 'lite_failed' };
  } catch (err) {
    return {
      nodes: [], droppedNodes: [], visibilityMismatches: [],
      extractedVia: 'lite_failed',
      liteError: err.message?.slice(0, 100) ?? 'unknown',
    };
  }
}
