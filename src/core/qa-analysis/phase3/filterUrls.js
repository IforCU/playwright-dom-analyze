...
export function filterUrls(candidates, config = {}) {
  ...
  for (const candidate of candidates) {
    ...
    if (keyToIndex.has(dedupKey)) {
      // Already have a representative — accumulate variant (cap at 5 unique samples)
      const idx = keyToIndex.get(dedupKey);
      const variants = results[idx].discoveredVariants;
      if (variants.length < 5 && !variants.includes(normalizedUrl)) {
        variants.push(normalizedUrl);
      }
      continue;
    }
