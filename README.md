...
| 단계 | 이름 | 상태 |
|---|---|---|
| Phase 0 | 초기 페이지 안정화 (광고, 모달 등 무효화) | ✅ 구현됨 |
...
### Phase 0: 초기 페이지 안정화 (Page Stabilization)

페이지 진입 직후, 본격적인 DOM 추출 직전에 실행됩니다. 화면을 뒤덮는 모달, 광고, 쿠키 동의 배너 등을 무력화하여 깨끗한 기준선(baseline)을 확보합니다.
*   **개선점 반영:** 이전 버전에서는 `body`, `html` 노드까지 높은 화면 비율(100% 이상)로 인해 차단 요소(blocker)로 오인되는 문제가 있었습니다. 이를 개선하여 루트 요소가 오탑되는 현상을 픽스했습니다.
*   단계별 조치:
    1.  **안전 닫기:** `닫기`, `건너뛰기` 등의 텍스트를 가진 버튼을 우선 클릭. (구매/가입 등 진행형 버튼은 무시)
    2.  **Escape 키:** 클릭할 버튼이 없으면 `Esc` 키 전송.
    3.  **미디어 제어:** 자동 재생 비디오/오디오 강제 일시 정지 및 음소거.
    4.  **CSS 숨김:** 높은 신뢰도의 방해 요소가 남으면 `visibility: hidden` 처리.
